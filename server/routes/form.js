    ...
    if (
      ...
      const t = forms.filter((f) => {
        if (
          ...
          (f.location.city.toLowerCase().includes(req.query.place) ||
            f.location.country.toLowerCase().includes(req.query.place))
        ) {
          return f;
        }
        if (
...
app.patch("/forms/:id", auth, async (req, res) => {
  // Update form data
  const _id = req.params.id;
  const updates = Object.keys(req.body);
  const nonallowedChanges = ["owner"];
  let isInvalidOpreation = updates.every((update) =>
    nonallowedChanges.includes(update)
  );
...
app.delete("/forms/:id", auth, async (req, res) => {
  try {
    let form = await Form.findOneAndDelete({
      _id: req.params.id,
      owner: req.user._id,
    });
    if (!form) {
